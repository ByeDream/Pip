...
- **TIERED MODEL REGISTRY**: Concrete model identifiers live only in `.env` as `MODEL_T0` / `MODEL_T1` / `MODEL_T2` (strongest → cheapest). Every call site picks a tier (`t0` / `t1` / `t2`) — never a model name — and resolves through `pip_agent.models.resolve_chain`. Background tasks are pinned to tiers in code (heartbeat / cron `t2`, reflect / consolidate `t1`, axioms `t0`); the main agent's tier comes from `persona.md`'s `model:` field. On a model-invalid error the chain steps DOWN to the next tier; never up. Other failures (rate limit, auth, network) propagate unchanged — Claude Code itself owns retry/backoff.
