...
def _task_tools(ctx: McpContext) -> list[SdkMcpTool]:
    ...
    async def task_create(args: dict[str, Any]) -> dict[str, Any]:
        pm = _pm()
        if pm is None:
            return _error("Task manager not available.")
        try:
            return _text(pm.create(args.get("story"), args.get("tasks", [])))
        ...
        for entry in tasks:
            ...
            if status == "merged" and wt is not None and story is not None:
                ...
                if owner and owner != "lead" and wt.exists(owner):
                    ...
                    if not result.ok:
                        try:
                            pm.update(story, [{"id": task_id, "status": "failed"}])
                        except ValueError:
                            pass
            ...
            if status == "completed" and wt is not None and story is not None:
                ...
                if owner and owner != "lead" and wt.exists(owner):
                    wt.remove(owner)
        try:
            return _text(pm.update(story, tasks))
        except ValueError as e:
            return _error(str(e))
    ...
    async def task_list(args: dict[str, Any]) -> dict[str, Any]:
        ...
        if pm is None:
            return _error("Task manager not available.")
        return _text(pm.render(args.get("story")))

    async def task_remove(args: dict[str, Any]) -> dict[str, Any]:
        pm = _pm()
        if pm is None:
            return _error("Task manager not available.")
        try:
            return _text(pm.remove(args.get("story"), args.get("task_ids", [])))
        except ValueError as e:
            return _error(str(e))
    ...
    async def task_submit(args: dict[str, Any]) -> dict[str, Any]:
        ...
        if pm is None:
            return _error("Task manager not available.")
        for key in ("story", "task_id"):
            if key not in args:
                return _error(f"'{key}' is required.")
        try:
            result = pm.update(
                args["story"],
                [{"id": args["task_id"], "status": "in_review"}],
            )
        except ValueError as e:
            return _error(str(e))
        return _text(result)
    ...
    return [
        SdkMcpTool(
            name="task_create",
            description=(
                "Create stories or tasks. Omit 'story' to create stories; "
            ...
            input_schema={
                ...
                "properties": {
                    "story": {
                        "type": "string",
                        "description": "Story ID to add tasks to. Omit for stories.",
                    },
                    "tasks": {
                        "type": "array",
                        "description": "Array of new stories or tasks.",
                        "items": {
                            "type": "object",
                            "properties": {
                                "id": {"type": "string"},
        ...
        SdkMcpTool(
            ...
            description=(
                ...
                "Provide 'story' to update tasks within it."
            ),
            input_schema={
                "type": "object",
                "properties": {
        ...
        SdkMcpTool(
            name="task_list",
            description="Show the task graph. Omit 'story' for overview.",
            input_schema={
                "type": "object",
                "properties": {
                    "story": {
                        "type": "string",
                        "description": "Story ID. Omit for overview.",
                    },
                },
            },
            handler=task_list,
        ...
        SdkMcpTool(
            name="task_remove",
            description="Remove stories or tasks. Fails if other items depend on them.",
            input_schema={
                "type": "object",
                "properties": {
                    "story": {"type": "string"},
        ...
        SdkMcpTool(
            name="task_submit",
            description="Submit completed work for review (subagent only).",
            input_schema={
                "type": "object",
                "properties": {
                    "story": {"type": "string"},
...
def _team_tools(ctx: McpContext) -> list[SdkMcpTool]:

    def _tm():
        return ctx.team_manager

    async def team_spawn(args: dict[str, Any]) -> dict[str, Any]:
        tm = _tm()
        if tm is None:
            return _error("Team manager not available.")
        for key in ("name", "prompt", "model", "max_turns"):
            if key not in args:
                return _error(f"'{key}' is required.")
        return _text(tm.spawn(
            args["name"], args["prompt"],
            model=args["model"], max_turns=args["max_turns"],
        ))

    async def team_send(args: dict[str, Any]) -> dict[str, Any]:
        tm = _tm()
        if tm is None:
            return _error("Team manager not available.")
        extra: dict[str, Any] = {}
        for key in ("req_id", "approve"):
            if key in args:
                extra[key] = args[key]
        return _text(tm.send(
            args.get("to", ""), args.get("content", ""),
            args.get("msg_type", "message"), **extra,
        ))
    ...
    async def team_status(args: dict[str, Any]) -> dict[str, Any]:
        ...
        if tm is None:
            return _error("Team manager not available.")
        return _text(tm.status())

    async def team_read_inbox(args: dict[str, Any]) -> dict[str, Any]:
        tm = _tm()
        if tm is None:
            return _error("Team manager not available.")
        inbox = tm.read_inbox()
        return _text(json.dumps(inbox, indent=2) if inbox else "(no messages)")

    async def team_list_models(args: dict[str, Any]) -> dict[str, Any]:
        tm = _tm()
        if tm is None:
            return _error("Team manager not available.")
        return _text(tm.list_models())
    ...
    async def team_create(args: dict[str, Any]) -> dict[str, Any]:
        ...
        for key in ("name", "description", "system_prompt"):
            if key not in args:
                return _error(f"'{key}' is required.")
        return _text(tm.create_teammate(args["name"], args["description"], args["system_prompt"]))

    async def team_edit(args: dict[str, Any]) -> dict[str, Any]:
        tm = _tm()
        if tm is None:
            return _error("Team manager not available.")
        name = args.get("name", "")
        ...
        return _text(tm.edit_teammate(name, **updates))

    async def team_delete(args: dict[str, Any]) -> dict[str, Any]:
        tm = _tm()
        if tm is None:
            return _error("Team manager not available.")
        name = args.get("name", "")
        if not name:
            return _error("'name' is required.")
        return _text(tm.delete_teammate(name))
    ...
    return [
        SdkMcpTool(
            name="team_spawn",
            description="Spawn a teammate to work on a task.",
            input_schema={
                "type": "object",
                "properties": {
                    "name": {"type": "string"},
        ...
        SdkMcpTool(
            name="team_send",
            description="Send a message to a working teammate.",
            input_schema={
                "type": "object",
                "properties": {
                    "to": {"type": "string"},
        ...
        SdkMcpTool(
            name="team_status",
            description="Show teammate roster and status.",
            input_schema=_EMPTY,
            handler=team_status,
        ),
        SdkMcpTool(
            ...
            input_schema=_EMPTY,
            handler=team_read_inbox,
        ),
        SdkMcpTool(
            name="team_list_models",
            description="List available models for teammates.",
            input_schema=_EMPTY,
            handler=team_list_models,
        ),
        SdkMcpTool(
            name="team_create",
            description="Create a new teammate definition.",
            input_schema={
                "type": "object",
                "properties": {
                    "name": {"type": "string"},
        ...
        SdkMcpTool(
            name="team_edit",
            description="Edit an existing teammate definition.",
            input_schema={
                "type": "object",
                "properties": {
                    "name": {"type": "string"},
        ...
        SdkMcpTool(
            name="team_delete",
            description="Delete a teammate definition.",
            input_schema={
                "type": "object",
...
def _skill_tools(ctx: McpContext) -> list[SdkMcpTool]:

    async def load_skill(args: dict[str, Any]) -> dict[str, Any]:
        if ctx.skill_registry is None:
            return _error("Skill registry not available.")
        name = args.get("name", "")
        if not name:
            return _error("'name' is required.")
        return _text(ctx.skill_registry.load(name))

    return [
        SdkMcpTool(
            name="load_skill",
            description="Load detailed domain instructions for a specific skill.",
            input_schema={
                "type": "object",
                "properties": {
