...
`WECOM_BOT_ID`/`WECOM_BOT_SECRET` are present, WeChat auto-started only
when a valid tier-3 `account_id=...` binding already exists; first-time
scans bootstrap lazily via `/wechat add <agent_id>`), a CLI-only boot
pays neither cost.
